...
import { paymentKeyboard } from "../keyboards/payment.keyboard";
import { superAdminKeyboard } from "../keyboards/superadmin.keyboard";
import { onlineVoteOperators, onlinePaymentOperators } from "../realtime";
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (ctx.session.role === "VOTE_OPERATOR") {
        if (ctx.from?.id) {
          onlineVoteOperators.set(ctx.from.id, ctx.from.id.toString());
        }

        await ctx.reply("👨‍💼 Operator panel", operatorKeyboard());
      } else if (ctx.session.role === "PAYMENT_OPERATOR") {
        if (ctx.from?.id) {
          onlinePaymentOperators.set(ctx.from.id, ctx.from.id.toString());
        }

        await ctx.reply("💳 Payment Operator panel", paymentKeyboard());
      } else if (ctx.session.role === "SUPER_ADMIN") {
        await ctx.reply("🛡 Super Admin Panel", superAdminKeyboard());
      } else if (ctx.session.role === "USER") {
        await ctx.reply("✅ Login muvaffaqiyatli!", userKeyboard());
      } else {
